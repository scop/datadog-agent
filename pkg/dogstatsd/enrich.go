...
var (
	...
	cardinalityTagPrefix = "dd.internal.cardinality:"
)

func extractTagsMetadata(tags []string, defaultHostname string, originTags string, entityIDPrecedenceEnabled bool) ([]string, string, string, string, collectors.TagCardinality) {
	...
	return tags, host, origin, k8sOrigin, taggerCardinality(cardinality)
}
...
func isExcluded(metricName, namespace string, namespaceExcluded []string) bool {
	if namespace != "" {
		for _, prefix := range namespaceExcluded {
...
func enrichMetricSample(metricSamples []metrics.MetricSample, ddSample dogstatsdMetricSample, namespace string, namespaceExcluded []string,
	...
	if !isExcluded(metricName, namespace, namespaceExcluded) {
