...
func (p *Probe) handleEvent(CPU uint64, data []byte) {
	...
	case model.ArgsEnvsEventType:
		...
		if event.ArgsEnvs.IsArgs {
			p.resolvers.ProcessResolver.UpdateArgs(event.ArgsEnvs.ID, event.ArgsEnvs.Array)
		} else {
			p.resolvers.ProcessResolver.UpdateEnvs(event.ArgsEnvs.ID, event.ArgsEnvs.Array)
		}
